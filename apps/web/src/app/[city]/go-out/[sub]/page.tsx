...
export async function generateMetadata({
  ...
  const [cityRow, subRow] = await Promise.all([
    ...
    query<SubcategoryRow>(`SELECT id, slug, name FROM listing_subcategories WHERE slug = $1 AND main_category = 'go-out' LIMIT 1`, [sub])
      .then((r) => r[0] ?? null),
  ]);
  ...
  return {
    title, description,
    alternates: { canonical: `https://muzgram.com/${city}/go-out/${sub}` },
    openGraph: { title, description, url: `https://muzgram.com/${city}/go-out/${sub}`, siteName: 'Muzgram' },
  };
}

export default async function GoOutSubcategoryPage({
  params,
}: {
  params: Promise<{ city: string; sub: string }>;
}) {
  ...
  const schema = buildBreadcrumbSchema([
    ...
    { name: subData.name, url: `https://muzgram.com/${city}/go-out/${sub}` },
  ]);
