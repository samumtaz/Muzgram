...
export async function generateMetadata({ params }: { params: Promise<{ city: string }> }): Promise<Metadata> {
  ...
  if (!cityRow) return {};
  const title = categoryTitle('eat', cityRow.name);
  const description = `Discover halal restaurants, cafés, and Muslim-owned eateries in ${cityRow.name}. Verified halal food near you. Download Muzgram.`;
  return {
    title,
    description,
    alternates: { canonical: `https://muzgram.com/${city}/eat` },
    openGraph: { title, description, url: `https://muzgram.com/${city}/eat`, siteName: 'Muzgram' },
  };
}

export default async function EatCategoryPage({ params }: { params: Promise<{ city: string }> }) {
  const { city } = await params;

  const [cityRows, listings] = await Promise.all([
    query<CityRow>(`SELECT id, slug, name FROM cities WHERE slug = $1 AND launch_status = 'active' LIMIT 1`, [city]),
    withCache(`seo:cat:eat:${city}`, 3600, () =>
      query<ListingRow>(`
        ...
        WHERE c.slug = $1 AND l.main_category = 'eat' AND l.is_active = true
  ...
  const schema = buildBreadcrumbSchema([
    ...
    { name: 'Halal Restaurants', url: `https://muzgram.com/${city}/eat` },
  ]);
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mt-6 mb-10">
          <h1 className="text-3xl sm:text-4xl font-bold text-text-primary mb-2">
            Halal Restaurants in {cityData.name}
          </h1>
          <p className="text-text-secondary text-lg max-w-2xl">
            Find halal-certified restaurants, cafés, shawarma spots, and Muslim-owned eateries in {cityData.name}.
          </p>
        ...
        {listings.length > 0 ? (
          ...
          <div className="text-center py-20">
            <p className="text-text-muted text-lg">No listings yet in {cityData.name}.</p>
            <p className="text-text-muted text-sm mt-2">
              Know a halal spot? Help us grow the community.
            </p>
          </div>
        )}
        ...
        <section className="mt-16 text-center">
          <p className="text-text-secondary mb-4">
            Find halal restaurants on the go with the Muzgram app.
          </p>
